 ...
 */
export interface StackStatus { stack: string; installed: boolean; commit: string; subject: string; branch: string; behind: number | "?"; dir: string; error?: string }
  ...
  try {
    return await withSession(deps, server, async (s, srv) => {
      ...
      for (const st of names) {
        ...
        try {
          const p = await probeStack(s, dir);
          out.push({ stack: st, installed: p.installed, commit: p.commit, subject: p.subject, branch: p.branch, behind: p.behind === "?" ? "?" : Number(p.behind), dir });
        } catch (e) {
          out.push({ stack: st, installed: false, commit: "", subject: "", branch: "", behind: "?", dir, error: (e as Error).message });
        }
      }
      return out;
    });
  } catch (e) {
    return names.map((st) => ({ stack: st, installed: false, commit: "", subject: "", branch: "", behind: "?" as const, dir: "", error: (e as Error).message }));
  }
}
