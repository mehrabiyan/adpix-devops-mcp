...
// ============================================================ i18n
const STR = {
  en: { nav: { dashboard: "Dashboard", assistant: "Assistant", servers: "Servers", ha: "High availability", databases: "Databases", backups: "Backups", deploys: "Deploys", dns: "DNS & connect", monitoring: "Monitoring", jobs: "Jobs & audit", security: "Security", settings: "Settings" },
  ...
  fa: { nav: { dashboard: "داشبورد", assistant: "دستیار", servers: "سرورها", ha: "دسترس‌پذیری بالا", databases: "پایگاه‌داده", backups: "پشتیبان‌گیری", deploys: "استقرار", dns: "DNS و اتصال", monitoring: "پایش", jobs: "کارها و ممیزی", security: "امنیت", settings: "تنظیمات" },
...
SCREENS.serverDetail = (c) => {
  ...
  cont.querySelector(".cc").innerHTML = known.map((svc) => `<div style="background:var(--c-card);padding:12px 14px"><div style="display:flex;align-items:center;gap:8px;margin-bottom:8px"><span data-dot="${svc}" style="width:8px;height:8px;border-radius:50%;background:var(--c-idle);flex:none;transition:background .25s" title="status pending"></span><span style="font-family:var(--font-mono);font-size:13px;font-weight:500">${esc(svc)}</span><span class="cstate" data-st="${svc}" style="margin-inline-start:auto;font-size:10.5px;color:var(--c-hint)"></span></div><div style="display:flex;gap:5px"><button class="iconbtn-sm" data-svc="${svc}" data-a="restart" style="flex:1" title="Restart">${ic("restart", 13)}</button><button class="iconbtn-sm" data-svc="${svc}" data-a="stop" style="flex:1" title="Stop">${ic("stop", 12)}</button><button class="iconbtn-sm" data-svc="${svc}" data-a="status" style="flex:1" title="Status">${ic("wave", 13)}</button></div></div>`).join("");
  ...
  // live container status (was a static grey list — never reflected the real server)
  const loadContainers = async () => {
    try {
      const { containers = [], error } = await api("/api/containers?server=" + encodeURIComponent(name));
      if (error) return;
      const bySvc = {}; for (const ct of containers) if (!bySvc[ct.service]) bySvc[ct.service] = ct; // docker ps -a is newest-first → current container wins
      cont.querySelectorAll("[data-dot]").forEach((d) => { const ct = bySvc[d.dataset.dot]; d.style.background = `var(--c-${ct ? ct.level : "idle"})`; d.title = ct ? (ct.status || ct.state) : "not present"; });
      cont.querySelectorAll(".cstate").forEach((e) => { const ct = bySvc[e.dataset.st]; e.textContent = ct ? (ct.health || ct.state) : "absent"; });
...
function certManagerCard() {
  ...
  const refresh = async () => {
    try {
      ...
      list.innerHTML = certs.length ? certs.map((ct) => `<div style="display:flex;align-items:center;gap:8px;padding:8px 0;border-bottom:1px solid var(--c-divider)"><div style="flex:1;min-width:0"><div style="font-family:var(--font-mono);font-size:13px">${esc(ct.domain)}</div><div class="muted" style="font-size:11px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">covers ${esc((ct.sans || [ct.domain]).join(", "))}</div><div class="muted" style="font-size:11px">${ct.notAfter ? `expires ${esc(String(ct.notAfter))}${typeof ct.daysLeft === "number" ? ` · <span style="color:var(--c-${ct.daysLeft < 14 ? "neg" : ct.daysLeft < 30 ? "warn" : "pos"})">${ct.daysLeft}d</span>` : ""}` : "unparsed"} · ${ct.hasChain ? "chain ✓" : "no chain"}</div></div><button class="btn btn-sm cpush" data-d="${esc((ct.sans && ct.sans[0]) || ct.domain)}">Install…</button><button class="iconbtn-sm cdel" data-d="${esc(ct.domain)}" title="Delete">${ic("stop", 12)}</button></div>`).join("") : `<div class="muted" style="font-size:12px">No certificates stored.</div>`;
      ...
      list.querySelectorAll(".cpush").forEach((b) => (b.onclick = () => action("cert_install", /\*/.test(b.dataset.d) ? {} : { domain: b.dataset.d })));
    } catch { /* */ }
  };
